<?php
  ...
  function check_if_already_liked($likerId, $postId){
    $conn = get_db_conn();
    
    $sql = "SELECT id FROM camagru.likes WHERE picture_id=? AND user_id=?";
    ...
    if ($result && $result->num_rows > 0) {
      $stmt->close();
      quit(json_encode(array("message"=> "Post already liked")), 200);
    }
    $stmt->close();
    $conn->close();
  }
  ...
  try{
    ...
    check_if_already_liked($likerId, $postId);
    
    $conn = get_db_conn();
    
    $sql = "INSERT INTO camagru.likes (picture_id, user_id) VALUES (?, ?)";
    $stmt = $conn->prepare($sql);
    ...
    $stmt->close();  
    quit(json_encode(array("message"=>"Post liked successfully")));
  }
